...
function decompress_compressed_block(data::Vector{UInt8}, ctx::FrameContext, history::Vector{UInt8})
    ...
    # 1. Literals Section
    literals, bytes_read = decode_literals(io, ctx)
...
function decode_literals(io::IO, ctx::FrameContext)
    ...
    if literals_block_type == 0 || literals_block_type == 1 # Raw or RLE
        ...
        if literals_block_type == 0 # Raw
            literals = read(io, regenerated_size)
            return literals, header_size + regenerated_size
        elseif literals_block_type == 1 # RLE
            rle_byte = read(io, UInt8)
            return fill(rle_byte, regenerated_size), header_size + 1
        end
    elseif literals_block_type == 2 || literals_block_type == 3 # Compressed or Treeless
        ...
        elseif size_format == 3
            ...
            b4 = read(io, UInt8)
            b5 = read(io, UInt8)
            regenerated_size = (UInt32(b1) >> 4) | (UInt32(b2) << 4) | ((UInt32(split_b3(b3, 0x3f)) & 0x3f) << 12)
            # wait, I'll just use the bitwise logic directly
        ...
        if literals_block_type == 2 # Compressed
            ...
            huffman_tree_size = position(io) - start_pos
            compressed_size -= huffman_tree_size
        end
        ...
        stream_data = read(io, compressed_size)
        if num_streams == 1
            literals = decode_huffman_stream(stream_data, ctx.huffman_table, Int(regenerated_size))
        else
            literals = decode_huffman_streams(stream_data, ctx.huffman_table, Int(regenerated_size))
        end
        
        return literals, header_size + compressed_size + huffman_tree_size
    end
end
